...
### OCR settings (for scanned/image-based PDF pages)

Also defined in `backend/config.py`:

- `OCR_MIN_TEXT_LENGTH = 20` — pages with less extracted text than this are treated as scanned and sent to OCR
- `OCR_LANGUAGE = "eng"` — Tesseract language code(s)
- `OCR_DPI = 300` — resolution used when rendering a scanned page to an image
- `POPPLER_PATH = None` — set this to your Poppler `bin` folder if it isn't on your system `PATH` (common on Windows)

---

## OCR System Requirements (not installed via pip)

OCR support requires two system-level programs in addition to the Python packages in `requirements.txt`. These are **not** Python packages and cannot be installed with `pip`:

1. **Tesseract OCR engine** — the actual OCR engine that `pytesseract` calls into.
   - Windows: install from the [Tesseract at UB Mannheim build](https://github.com/UB-Mannheim/tesseract/wiki), then make sure `tesseract.exe` is on your `PATH` (or point `pytesseract.pytesseract.tesseract_cmd` to it).
   - macOS: `brew install tesseract`
   - Linux (Debian/Ubuntu): `sudo apt-get install tesseract-ocr`

2. **Poppler** — provides the `pdftoppm`/`pdftocairo` utilities that `pdf2image` uses to render PDF pages into images before OCR.
   - Windows: download a Poppler release (e.g. from [oschwartz10612/poppler-windows](https://github.com/oschwartz10612/poppler-windows/releases)), unzip it, and either add its `bin` folder to `PATH` or set `POPPLER_PATH` in `backend/config.py` to that `bin` folder.
   - macOS: `brew install poppler`
   - Linux (Debian/Ubuntu): `sudo apt-get install poppler-utils`

Once both are installed, `pip install -r requirements.txt` (which now includes `pytesseract` and `pdf2image`) gives you full OCR support with no other code changes needed.
...
3. `PyPDFLoader` reads pages into documents.
   - If a page has little or no extractable text (a scanned/image-based page), it is automatically rendered to an image (`pdf2image`/Poppler) and run through Tesseract OCR (`pytesseract`) instead, so its text still makes it into the same `Document` object. Normal text pages are unaffected.
